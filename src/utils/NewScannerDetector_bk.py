...
from src.utils.LoggerDetector import logger, LoggerDetector
...
class NewScannerDetector:
    ...
    def _apple_twain_setting(self) -> None:
        ...
        try:
            src.set_capability(twain.CAP_AUTOMATICSENSEMEDIUM, twain.TWTY_BOOL, False)
            print("关闭自动感应介质/检测ADF是否有纸成功")
        except Exception as e:
            print("关闭自动感应介质/检测ADF是否有纸; 失败")
    ...
    def _acquire_images(self) -> list:
        p = self._params
        # if p.scan_file_pages > 0:
        #     try:
        #         self._source.set_capability(twain.CAP_XFERCOUNT, twain.TWTY_INT16, 4)
        #         logger.info(f"设置扫描页数：{4}")
        #     except Exception as exc:
        #         logger.error(f"设置扫描页数失败：{exc}")

        self._source.request_acquire(show_ui=False, modal_ui=False)
        time.sleep(4)
        logger.info(f"正在唤醒扫描仪马达, 等待中·····")
        page = 0
        try:
            while True:
                ...
                try:
                    ...
                    print(f"handle={handle}, count={count}")
                except twain.exceptions.excTWCC_PAPERJAM:
                    logger.warning("扫描终端, 最后一页正常退出")
                    break
